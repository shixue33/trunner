...
public class SendMail {
	...
	public SendMail(Test test,Config config){
		this.test = test ;
		this.config = config;		
	}
	...
	public void send(){
		...
		mailInfo.setUserName(config.mailUserName);
		mailInfo.setPassword(config.mailUserPassword);// 您的邮箱密码
		mailInfo.setFromAddress(config.mailUserName);
		
		String mailto = config.mailToAddress;
		String[] mailtos = mailto.split(";");
		
		mailInfo.setToAddress(mailtos);
		mailInfo.setSubject("test-no-"+test.getNo());
		//
		String filePath = "html/"+String.valueOf(test.getNo())+".html";
		String c = "";
		try{
